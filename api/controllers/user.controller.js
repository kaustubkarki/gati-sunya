...
export const savePost = async (req, res) => {
  ...
  try {
    const savedPost = await prisma.savedPost.findUnique({
      ...
      include: {
        user: true, // Include the user details who saved the post
      },
    });
    ...
    if (savedPost) {
      ...
      res.status(200).json({ message: "Post removed from saved list", user: savedPost.user });
    } else {
      ...
      res.status(200).json({ message: "Post saved", user: newSavedPost.user });
    }
  } catch (err) {
    console.log(err);
